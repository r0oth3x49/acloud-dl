...
class CloudGuru(WebVtt2Srt, ProgressBar, GetPass):
    ...
    @staticmethod
    def courses_not_downaloaded(coursesList, path="", isFiltering=False):
    ...
    def courses_downloaded(self, path='', download_all=False, download_only_new=False):
        ...
        courses = self.courses_not_downaloaded(acloud.courses(cookies=self.cookies), path, download_only_new)
...
def main():
    ...
    if not options.cookies:
        ...
        if os.path.isfile(filename):
            ...
            cloudGuru = CloudGuru(cookies=cookies)

            if options.info:
                cloudGuru.courses_downloaded(path=options.output, download_only_new=options.download_only_new)

            if not options.info:
                cloudGuru.course_download(path=options.output, quality=options.quality, user_extension=options.extension,
